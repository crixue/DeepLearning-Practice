...
from com.xrj.learning.tensorFlowPractice import tf_utils
import scipy
...
def forward_propagation(X, parameters):
    ...
    # Z1_reg = tf.nn.dropout(Z1, keep_prob=keep_prob)
    A1 = tf.nn.relu(Z1)  # A1 = relu(Z1)
    # A1 = tf.nn.relu(Z1_reg)
    Z2 = tf.add(tf.matmul(W2, A1), b2)  # Z2 = np.dot(W2, a1) + b2
    # Z2_reg = tf.nn.dropout(Z2, keep_prob=keep_prob)
...
if __name__ == '__main__':
    ...
    # loss = tf.Variable((y_hat - y)**2, dtype="float32", name="loss")
    # init = tf.global_variables_initializer()
    ...
    # sess = tf.Session()
    # print(sess.run(c))
    # x = tf.placeholder(dtype="int64", name="x")
    # with tf.Session() as sess:
