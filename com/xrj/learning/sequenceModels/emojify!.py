import numpy as np
from com.xrj.learning.sequenceModels.emo_utils import *
import emoji
...
def Emojify_V2(input_shape, word_to_vec_map, word_to_index):
    ...
    X = LSTM(units=128, activation='tanh', return_sequences=True)(embeddings)
    # Add dropout with a probability of 0.5
    X = Dropout(0.5)(X)
    # Propagate X trough another LSTM layer with 128-dimensional hidden state
    # Be careful, the returned output should be a single hidden state, not a batch of sequences.
    X = LSTM(units=128, activation='tanh', return_sequences=False)(X)
    # Add dropout with a probability of 0.5
    X = Dropout(0.5)(X)
...
if __name__ == '__main__':
    ...
    loss, accur = model.evaluate(X_test_indices, Y_test_oh)
